...
/**
 * Calculates the checkout statistics (order count, order amount total) for the
 * last 60 seconds.
 *
 */
@Component
public class CheckoutStatisticsCalculator {
	...
	private SortedMap<Long, PerSecondStatisticsEntry> checkoutSecondToStatistics = new TreeMap<>();

	/*
	 * Synchronization could be done more fine-grained if necessary, but this
	 * solution is really simple and performance tests show a performance which is
	...
	/**
	 * Ensures that even if "getStatistics" isn't called regularly, the statistics
	...
	/**
	 * Removes all entries from the map which are at least 60 seconds older than
	 * currentTimeSeconds. Updates the global statistics accordingly.
	 */
	private void removeOldEntries(long currentTimeSeconds) {
		long threshold = currentTimeSeconds - STATISTICS_PERIOD_SECONDS;
		SortedMap<Long, PerSecondStatisticsEntry> toRemove = checkoutSecondToStatistics.headMap(threshold);
